...
test('expiry reminder rejects invalid custom days and dashboard consumes the saved setting', async () => {
  ...
  assert.match(dashboardSource, /sort\(\(left, right\) => daysUntil\(left\.expireDate\) - daysUntil\(right\.expireDate\)\)/)
  assert.match(dashboardSource, /expireLabel: expireStatus === 'expired' \? `已过期 \$\{item\.expireDate\}` : `快过期 \$\{item\.expireDate\}`/)
  assert.match(dashboardTemplate, /{{expiryReminderDays}} 天内到期药品/)
  assert.match(dashboardTemplate, /{{item\.expireLabel}}/)
  assert.doesNotMatch(dashboardTemplate, /warn-pill">{{item\.expireDate}}/)
