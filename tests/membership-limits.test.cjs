...
test('membership center presents the multi-family benefit and account-level usage scope', () => {
  ...
  assert.match(template, /基础版快速记录 3 次（累计）/)
  assert.match(script, /快速记录不限次数/)
  assert.match(script, /畅享版[\s\S]*安心版/)
...
test('membership card keeps redemption and displays server-priced plans with a payment gate', () => {
  const script = fs.readFileSync(path.join(root, 'miniprogram/pages/membership/index.js'), 'utf8')
  const template = fs.readFileSync(path.join(root, 'miniprogram/pages/membership/index.wxml'), 'utf8')
  assert.match(template, /id="redeem-section"/)
  assert.match(template, /输入会员兑换码/)
  assert.match(template, /item.benefitItems/)
  assert.match(template, /item.priceText/)
  assert.match(template, /disabled="\{\{!paymentReady \|\| loading\}\}"/)
  assert.doesNotMatch(script, /DEFAULT_PLANS/)
})
