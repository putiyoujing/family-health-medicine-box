...
test('family member limit and account labels are explicit before submitting', () => {
  ...
  assert.match(source, /升级会员后可添加至 10 位/)
...
test('membership plans render immediately and record lists explain completion', () => {
  const membershipSource = read('miniprogram/pages/membership/index.js')
  assert.match(membershipSource, /plans: decoratePlans\(DEFAULT_PLANS\)/)
