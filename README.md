...
- 当前修复候选版本：`1.1.1`（会员支付与兑换可靠性修复；待体验版上传、审核和正式发布）
...
- `cloudfunctions/paymentApi`：会员兑换、虚拟支付签名、平台查单及订单权益发放
...
- 1.0.14 已下线原 AI 查询入口；新的图片与语音解析流程尚未开放
- 病程复诊摘要导出
...
- 当前修复候选已实现虚拟支付签名、平台查单、可信发货回调及每 5 分钟订单对账。用户提供的公众平台截图已确认虚拟支付开通，AppID 与 OfferID 已核验；AppKey、AppSecret、四个已发布商品 ID 的云函数配置、函数部署、消息推送/定时触发器、数据库索引及真机验收仍待完成，正式收款保持关闭。套餐展示不代表真实支付已验收。
- 生产环境、5 个云函数、24 个 `ADMINONLY` 集合、管理后台静态托管及提醒定时触发器已核验。
- 1.0.13 Web 管理后台已部署；公网入口和活动资源均返回 200，线上构建已包含中性会员兑换提示；无管理员会话的浏览器只显示登录表单，未加载任何业务数据。
- 小程序 1.0.14 已由项目负责人提供的微信公众平台版本管理截图确认于 2026-07-27 正式发布。
- 小程序 1.0.15 已上传微信体验版；微信审核和正式发布仍待完成。
- 隐私配置、双账号隔离、iOS/Android 真机和真实提醒触达已由项目负责人于 2026-07-24 确认通过；原始截图、日志或测试记录未纳入仓库。
