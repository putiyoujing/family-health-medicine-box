...
管理后台已经达到 `deployed + live verified（已登录和无会话两种状态）`。1.0.13 小程序已提交微信审核、结果待定；公开发布仍为 `NO-GO`：隐私、双账号、iOS/Android 真机和真实提醒触达尚未形成闭环证据，且尚无审核通过或正式发布证据；AppSecret 轮换为负责人延期事项。
