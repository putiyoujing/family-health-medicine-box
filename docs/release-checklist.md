...
11. 导出家庭数据和就医沟通记录。
12. 进入会员中心，确认不显示套餐价格、购买渠道或支付入口；输入会员兑换码并完成权益激活。
...
截至 2026-07-24，1.0.13 审核修订候选已移除小程序套餐价格、购买渠道、旧结算/优惠券页面和反馈联系方式采集；会员兑换入口保留。`npm run check` 已通过，生产 `healthApi`、`paymentApi`、`adminApi` 已重新部署为 `Active/Available`，会员兑换提示配置及 `paymentApi.getPlans` 实际返回均为“请输入已有会员兑换码完成权益激活。”。1.0.13 Web 管理后台也已部署到生产 `/admin/`，活动资源、公网响应、新旧文案与浏览器登录页均已核验。项目负责人已确认隐私、双账号、iOS/Android 真机和真实提醒触达全部通过，原始附件未纳入仓库；1.0.13 小程序已提交微信审核，当前结果待定，因此正式公开发布仍为 `NO-GO`，等待审核结果闭环。
