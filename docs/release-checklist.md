...
截至 2026-07-24，1.0.13 审核修订候选已移除小程序套餐价格、购买渠道、旧结算/优惠券页面和反馈联系方式采集；会员兑换入口保留。`npm run check` 已通过，生产 `healthApi`、`paymentApi`、`adminApi` 已重新部署为 `Active/Available`，会员兑换提示配置及 `paymentApi.getPlans` 实际返回均为“请输入已有会员兑换码完成权益激活。”。1.0.12 Web 管理后台仍是当前已核验的线上静态版本；1.0.13 小程序尚未上传，iOS/Android 真机截图、重新审核结果、隐私、双账号和真实提醒触达仍需留证，因此公开发布继续为 `NO-GO`，直到新证据闭环。
