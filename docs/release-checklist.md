...
截至 2026-08-22，小程序线上版本为 1.0.14；项目负责人提供的微信公众平台版本管理截图记录其于 2026-07-27 正式发布。1.0.15 为本地候选版本，包含页面缓存与刷新优化、药品图片重新打开后的回显修复、用药操作按钮紧凑化、隐藏未开放的图片整理入口，以及待接受家庭邀请的继续分享修复。后台优惠券与会员兑换码拆分、管理页状态保留和退役 AI 查询源码清理也随候选源码登记，但尚未部署云函数或后台。`npm run check` 已通过 165 项自动化测试和 22 项静态发布防护；未迁移数据库或改写生产数据。体验版上传、对应后台与云函数的单独部署验收、真机验收、微信审核和正式发布仍分别为 `PENDING`，完成前不得把 1.0.15 记为已上线。
