...
截至 2026-09-12，小程序正式线上版本仍为 1.0.14；1.0.15 已上传微信体验版，包含快速记录症状、图片整理确认、页面缓存与刷新优化、药品图片回显修复、家庭邀请继续分享修复及相关安全与会员逻辑。4 个有代码变化的 CloudBase 云函数和既有 `/admin/` 静态后台已更新并完成回读，GitHub 发布分支已推送。`npm run check` 已通过 201 项自动化测试和 22 项静态发布防护；未迁移数据库或写入生产健康数据。PC 端真机已完成授权登录，并实际打开“快速记录”页；为避免写入真实健康数据，停在提交前。微信审核和正式发布仍为 `PENDING`，完成前不得把 1.0.15 记为正式上线。
