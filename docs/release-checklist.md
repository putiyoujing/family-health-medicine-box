...
截至 2026-07-24，代码门禁和本机生产声明门禁通过；CloudBase 环境、函数、集合权限、托管与定时触发器已核验。1.0.12 Web 管理后台已部署到生产 `/admin/`，入口与新资源均返回 200，已登录浏览器完成实际渲染。匿名会话拒绝、AppSecret 轮换、隐私、双账号、iOS/Android 真机和真实提醒触达仍需补充可审计证据，因此公开发布仍为 `NO-GO`。本次明细见 [P0 发布证据记录（2026-07-24）](p0-release-evidence-2026-07-24.md)。图片识别与真实支付不在 1.0.12 首发范围，商业化采用兑换码。
