...
2. [开发结果总结](development-summary.md)：当前功能、后端、Web 管理后台、发布情况和已执行验证。
3. [上线准备工作清单](launch-prep-workplan.md)：上线前从账号、云开发、云函数、外部服务、审核材料到监控的工作拆解。
4. [发布检查清单](release-checklist.md)：发布前可逐项勾选的工程、隐私、医疗安全、测试、后台和 GitHub 检查。
5. [微信云开发数据库设计](wechat-cloud-database.md)：集合、权限、云函数、管理员配置、发布前配置和外部服务接入。
6. [Web 管理后台](web-admin.md)：本地打开、后台能力、后端接口和部署建议。
