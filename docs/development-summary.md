...
本项目是一个微信小程序产品，定位为「家庭健康记录与药箱管理」。核心目标是帮助家庭记录成员健康情况、管理家庭药箱、追踪用药记录，并为产品管理者提供后台数据统计。

项目包含三部分：

- 微信小程序前端：面向家庭用户使用。
- 微信云开发后端：负责用户身份、家庭数据、药箱、健康记录、用药记录和管理统计接口。
...
- `healthApi`：提供小程序核心业务接口，包括药箱、健康记录、用药记录、附件、统计和 AI 安全查询。
- `paymentApi`：提供会员套餐、优惠券、订单创建和模拟支付开通。
...
- 小程序内管理页通过 `admins` 集合校验管理员 openid。
...
- 数据列表：用户、家庭、订单、会员家庭、优惠券、AI 用量、药品、健康记录、用药记录、附件。
- 未配置真实接口时显示演示数据。
...
- 对用户端来说，小程序已覆盖家庭药箱、健康记录、用药记录和基础 AI 检索整理。
- 对商业化来说，已具备家庭共享、会员权益、订单、优惠券和模拟支付闭环。
- 对产品管理者来说，已有独立 Web 管理后台和小程序内管理页，可查看用户、家庭、订单、会员、优惠券和 AI 用量。
- 对工程交付来说，代码已上传 GitHub，Web 后台已发布到 GitHub Pages。

后续重点不是继续补页面，而是接入真实微信云开发环境、配置数据库权限、补齐正式运营资料，并进行真实设备测试。
