...
- 用药后自动扣减库存
- AI 助手基于本地记录进行检索式整理
- 会员中心支持兑换码激活、权益对比和用量展示，不显示套餐价格、购买渠道或支付入口
- 病程复诊摘要导出
...
仓库为公开 GitHub 仓库，当前开发分支通过 Pull Request 合并到 `master`。版本号同时用于 `package.json` 和小程序上传脚本，发布标签采用 `v1.0.13`。
...
- `npm run check`：构建、Lint、86 个 JavaScript 文件语法、23 个页面、5 个 Tab、29 个页面 JSON、62 个云函数动作、160 条自动化测试和 22 项静态发布保护。
- 1.0.13 审核修订已重新部署生产 `healthApi`、`paymentApi`、`adminApi`；三者均为 `Active/Available`，会员兑换提示配置和云函数实际返回已核验为中性文案。
- 项目负责人于 2026-07-24 确认 1.0.13 小程序已提交微信审核；当前状态为审核中，尚未取得通过或正式发布证据。
...
- 托管：1.0.13 已部署；生产 `/admin/` 与 `index-By3rh1sH.js` 均返回 200，线上资源哈希与本地构建一致，浏览器完成登录页实际渲染且控制台无新增错误。
...
- 隐私保护指引、双账号家庭共享与跨家庭隔离、iOS/Android 真机、真实订阅消息触达已由项目负责人于 2026-07-24 确认通过；原始截图、日志和设备记录未纳入仓库。
- 图片识别默认关闭；若后续开放，需要接入服务、成本控制、失败降级和用户确认验收。
- 1.0.13 小程序只保留兑换码激活，不展示套餐价格、购买渠道或支付入口。
- Git 全历史密钥扫描未发现 AppSecret 泄漏；项目负责人决定延期轮换，密钥值仍不得进入仓库、日志或知识库。
- 1.0.13 Web 管理后台已部署；无管理员会话的浏览器只显示登录表单，未加载业务数据。
