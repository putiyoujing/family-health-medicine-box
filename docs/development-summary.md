...
本项目是一个微信小程序产品，产品名为「家人健康记」，定位为家庭健康管理与记录工具。核心目标是帮助家庭记录成员健康情况、管理家庭常备健康用品、追踪用药记录，并为产品管理者提供后台数据统计。

项目包含三部分：

- 微信小程序前端：面向家庭用户使用。
- 微信云开发后端：负责用户身份、家庭数据、常备记录、健康记录、用药记录和管理统计接口。
...
- `healthApi`：提供小程序核心业务接口，包括常备记录、健康记录、用药记录、附件、统计和 AI 安全查询。
...
- 对用户端来说，小程序已覆盖家庭常备记录、健康记录、用药记录和基础 AI 检索整理。
