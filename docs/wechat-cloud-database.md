...
- 免费账号最多创建 1 个家庭；只有实际调用 `createFamily` 创建后才占用该名额。
...
- 创建家庭时自动创建“本人”成员档案并绑定 owner；既有线上未绑定 owner 在登录或读取家庭时幂等补齐。
...
首次进入时只创建 `users`。新用户的 `currentFamilyId` 为空，不自动创建 `families`、`family_members` 或 `family_roles`。

已有线上家庭角色的用户保持原数据和当前家庭；新用户可以接受邀请直接加入已有家庭，或在首次明确记录病程/添加药品时调用 `createFamily` 按需创建家庭、本人档案和 owner 角色。
...
- `createFamily` 由服务端校验创建额度；成功后自动创建并绑定 owner 的“本人”档案，并把新家庭设为当前家庭。它是唯一的家庭创建入口，首次记录病程或添加药品时复用该接口。
