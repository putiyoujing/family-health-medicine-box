...
- “优惠券表”独立生成优惠券码，只用于订单抵扣，并只读写 `coupons`。
- “兑换码批次表”和“会员兑换码表”独立批量生成会员兑换码，只读写 `coupon_code_batches` 与 `coupon_codes`。
- 会员兑换只记录兑换码、批次、订阅和兑换记录；不读取、写入或核销优惠券。已有历史记录仅保留审计，不做迁移。
